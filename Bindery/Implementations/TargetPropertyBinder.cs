...
namespace Bindery.Implementations
{
    internal class TargetPropertyBinder<TSource, TTarget, TProp> : ITargetPropertyBinder<TSource, TTarget, TProp>
    {
        ...
        private void ConfigureTargetPropertyUpdate<TSourceProp>(Expression<Func<TSource, TSourceProp>> sourceExpression,
            Action<TSourceProp> updateProperty)
        {
            ...
            var subscriptions = observables.Select(observable => GetObservableForSubscription(observable).Subscribe(updateProperty));
            subscriptions.ToList().ForEach(sub => _parent.AddSubscription(sub));
        }

        private IObservable<T> GetObservableForSubscription<T>(IObservable<T> observable)
        {
            return _parent.DefaultScheduler == null ? observable : observable.ObserveOn(_parent.DefaultScheduler);
        }
    }
}